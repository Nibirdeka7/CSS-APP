...
export const useAdminStore = create((set, get) => ({
    ...
    try {
      ...
      for (const event of events) {
        try {
          ...
          console.error(
            `Failed to fetch matches for event ${event._id}:`,
            error,
          );
        }
      }
  ...
  fetchEligibleTeams: async (eventId) => {
    set({ isLoading: true });
    try {
      // 🔴 WAS: const response = await eventAPI.getEligibleTeams(eventId);
      // 🟢 CHANGE TO:
      const response = await teamAPI.getEligibleTeams(eventId);
